...
| CI watched too early or interpreted too soon | PR checks are read while still pending and treated as final | `erp-ci-workflow` | `gh pr checks PR-NUMBER --watch --interval 15` | Required checks have not completed successfully | Keep waiting until every required check is `SUCCESS` |
| Backend CI duration misunderstood | A long-running backend job is mistaken for a hang | `erp-ci-workflow`, `erp-quality-gates` | `gh pr checks PR-NUMBER --watch --interval 15`; `gh run watch RUN_ID --interval 15` | Required backend quality job fails or is cancelled | Continue watching until terminal status is known, then fix only in scope or stop |
