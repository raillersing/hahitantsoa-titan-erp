...
Current next bundle: **Phase 1F-D — execute the approved multi-role acceptance matrix
against the official Django fixture and close remaining authentication/RBAC evidence
gaps**. Phase 1F-B/C strict real-backend acceptance is integrated with exact-SHA
evidence, including a successful two-test Desktop Chrome run against Django after
fixture migrations and CSRF-origin setup. Phase 1F-A automated role acceptance is
also integrated with exact-SHA evidence.
...
|---|---|---|---|---|---:|---|---|
| Authentication and session | Approved login UI and prototype shell gated by `AuthProvider`; intended hash destination preserved across login and structured protected navigation; read-only profile mounted from the real session identity | Real Django session API; no authentication bypass or business-auth fallback; authenticated identity is preserved during recoverable network loss and revalidated on recovery | `/api/v1/auth/session/`, `/api/v1/auth/login/`, `/api/v1/auth/logout/`; Django session and CSRF contracts | Backend auth contract tests plus frontend build, role-matrix Playwright 18/18, real-backend acceptance 2/2 in PR #485, FE-B/FE-C/FE-D approvals, and human UI validation proven through PRs `#466`, `#468`, `#470`, `#482`, `#483`, and `#485`; full multi-role denied-flow evidence remains pending | P1 | Real Django session persistence, logout, expiry, console and network behavior are proven; remaining gate is multi-role backend-authoritative denied-flow evidence and complete Vitest evidence | `1F-D` |
