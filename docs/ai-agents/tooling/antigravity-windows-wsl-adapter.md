...
- **Supported Modes:** The adapter currently supports `task-start`, `finalize-pr`, `repo-status`, `pr-checks`, and `pr-create`.
- **Not Enabled Yet:** Automated git actions such as branch creation (`create-branch`), local commit (`commit`), and remote push (`push`) are **not enabled yet** in this first adapter version. Attempting to run them via the adapter will result in an error or a protocol violation.
