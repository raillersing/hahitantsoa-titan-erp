...
@transaction.atomic
def update_hahitantsoa_event_draft_public_reference(
    *,
    ...
    old_ref = event_draft.public_reference
    if old_ref == new_ref:
        return event_draft

    # Check uniqueness
    if (
        HahitantsoaEventDraft.objects.filter(public_reference=new_ref, is_deleted=False)
        .exclude(id=event_draft.id)
        .exists()
    ):
        raise ValidationError(f"La référence '{new_ref}' est déjà utilisée par un autre événement.")

    # Update event draft
    event_draft.public_reference = new_ref
    if actor and getattr(actor, "is_authenticated", False):
        event_draft.updated_by = actor
    event_draft.save(update_fields=["public_reference", "updated_by", "updated_at"])

    # Cascade update all associated document instances
    instances = DocumentInstance.objects.filter(hahitantsoa_event_draft=event_draft)
    ...
    record_audit_event_on_commit(
        actor=actor,
        action="event_draft.public_reference_updated",
