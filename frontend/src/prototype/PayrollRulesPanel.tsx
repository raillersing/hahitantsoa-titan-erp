  ...
  return (
    <section className="mt-8 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm" aria-labelledby="payroll-rules-title">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        ...
        <button type="button" onClick={newDraft} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white">Nouveau brouillon</button>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[15rem_1fr]">
        ...
        <form onSubmit={save} className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-4">
            {(!selected || selected.status === "draft") && <button disabled={saving} className="rounded-lg bg-teal-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{saving ? "Enregistrement…" : "Enregistrer le brouillon"}</button>}
            {selected?.status === "draft" && <button type="button" disabled={saving} onClick={() => void runAction(submitPayrollRuleSet)} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 disabled:opacity-50">Soumettre à validation</button>}
            {selected?.status === "pending_review" && <button type="button" disabled={saving} onClick={() => void runAction(activatePayrollRuleSet)} className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Activer la configuration</button>}
            {selected && selected.status !== "active" && selected.status !== "archived" && <button type="button" disabled={saving} onClick={() => void runAction(archivePayrollRuleSet)} className="rounded-lg border border-red-200 px-4 py-2 text-sm font-semibold text-red-700 disabled:opacity-50">Archiver</button>}
          </div>
        </form>
      </div>
    </section>
