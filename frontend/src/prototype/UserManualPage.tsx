...
export default function UserManualPage({ onNavigate }: UserManualPageProps) {
  ...
  const sections = useMemo<ManualSection[]>(() => [
    ...
    },
    {
      ...
      content: <><p>Le parcours Hahitantsoa couvre les événements et la location du lieu, avec ou sans logistique.</p><ol className="mt-4 list-decimal space-y-2 pl-5"><li>Choisissez le client, puis <strong>Hahitantsoa</strong>.</li><li>Renseignez le type d’événement, les dates et heures via le calendrier, le lieu et le nombre d’invités.</li><li>Choisissez l’un des deux types : <strong>Location nue</strong> ou <strong>Location + logistique</strong>.</li><li>Avec la logistique, sélectionnez directement les articles ou construisez un pack personnalisable.</li><li>Contrôlez le résumé et le montant total, puis générez le proforma.</li><li>Après acompte et validation du contrat, le dossier devient une réservation confirmée selon les contrôles métier.</li></ol><Info title="Documents associés" tone="green">Le contrat Hahitantsoa peut être accompagné de la décharge de responsabilité et, dans le workflow logistique, de la checklist manuelle.</Info></>,
    },
    {
      id: "titan",
